...
Function New-RKVM {
  ...
  If (Get-VMSwitch $Network) { Write-Verbose "VM Switch $Network Exists" } else {
    ...
    New-VMSwitch -Name $Network -NetAdapterName "Ethernet2"
  ...
  $VHDPart = Get-Partition -DiskNumber $VHDDisk.Number | Where-Object -Property DriveLetter -EQ "W"
...
Set-VM -name $Name -ProcessorCount $CPUCounts
Set-VM -Name $name -AutomaticStartAction Nothing
Set-VM -Name $name -AutomaticStopAction ShutDown

Set-VMProcessor -VMName $Name -ExposeVirtualizationExtensions $NHV
...
#    Storage Server - for chapter 10 iSCSI target
